	...
	local hb = CreateFrame('Button', nil, WorldStateAlwaysUpFrame, 'SecureActionButtonTemplate')
    hb:SetFrameLevel(2)
    hb:SetAllPoints(h)
    hb:EnableMouse(true)
	hb:SetAttribute("type1", "macro")
	...
	local ab = CreateFrame('Button', nil, WorldStateAlwaysUpFrame, 'SecureActionButtonTemplate')
    ab:SetFrameLevel(2)
    ab:SetAllPoints(a)
    ab:EnableMouse(true)
	...
	WSGUIupdateFC = function(fc)
		...
		if flagCarriers['Alliance'] then
			...
			ab:SetAttribute("macrotext1", "/targetexact " .. flagCarriers['Alliance'])
		else
			a:SetText('')
			ah:SetText('')
		...
		if flagCarriers['Horde'] then
			...
			hb:SetAttribute("macrotext1", "/targetexact " .. flagCarriers['Horde'])
		else
			h:SetText('')
			hh:SetText('')
			fcHealth['Horde'] = nil
		...
		-- AlwaysUpFrame dynamic buttons are dynamic
		for i = 1, 3 do
			local dynButtonIcon = _G['AlwaysUpFrame' .. i .. 'DynamicIconButtonIcon']
			if dynButtonIcon then
				local dynIconText = dynButtonIcon:GetTexture()
				if(dynButtonIcon:IsShown() and dynIconText == SPELLINFO_WSG_FLAGS['Alliance']['worldFrameIcon']) then
					a:SetPoint('LEFT', _G['AlwaysUpFrame' .. i .. 'DynamicIconButton'], 'RIGHT', 4, 2)
				end
				
				if(dynButtonIcon:IsShown() and dynIconText == SPELLINFO_WSG_FLAGS['Horde']['worldFrameIcon']) then
					h:SetPoint('LEFT', _G['AlwaysUpFrame' .. i .. 'DynamicIconButton'], 'RIGHT', 4, 2)
				end
			end
		end
	...
	WSGUIinit = function(insideBG)
		if insideBG then
			--local hdb = _G['AlwaysUpFrame2DynamicIconButton']
			...
			--local adb = _G['AlwaysUpFrame3DynamicIconButton']
