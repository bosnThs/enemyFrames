	...
	if drf > 0 then		
		...
		for k, v in pairs(buffList) do
			if v.caster == tar and v.spell == b then
				...
				s = s and s or v.stacks		--new line
			end
		end
...
local combatlogParserOLD = function()	
...
local combatLogParserFunctions = {
	['SPELL_CAST_START'] = function()
		newCast(arg4, arg10, false)
		print('new cast: '.. arg4 .. ' -> ' .. arg10)
	end,
	['SPELL_CAST_FAILED'] = function()
		forceHideTableItem(casts, arg4)
		print('failed cast: ' .. arg4 .. ' -> ' .. arg10)
	end,
	['SPELL_CAST_SUCCESS'] = function()
		forceHideTableItem(casts, arg4)
		print('sucess cast: ' .. arg4 .. ' -> ' .. arg10)
	end
}

local combatlogParser = function()	
	if event == 'COMBAT_LOG_EVENT_UNFILTERED' and combatLogParserFunctions[arg2] then
...
SPELLCASTINGCOREgetPrioBuff = function(name, n)
	--local p = ENEMYFRAMECOREgetPlayer(name)['buffs']
	--local b = sortPriobuff(b, p)
...
SPELLCASTINGCOREaddBuff_NEW = function(a_target, a_buff, a_timeStart, a_timeEnd)
	table.insert(buffList, buff.create(a_target, a_buff, 1, SPELLINFO_BUFFS_TO_TRACK[a_buff], 1, getTimeMinusPing(), a_timeStart, a_timeEnd))
end
------------------------------------
