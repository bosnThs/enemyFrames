...
local 	enemyFrame = CreateFrame('Frame', 'enemyFrameDisplay', UIParent, 'SecureHandlerStateTemplate')
		...
		enemyFrame.bottom.raceButton:SetPoint('LEFT', enemyFrame.bottom.classButton, 'RIGHT', 8, 0)
...
--	enemyFrame.efcButton.flagTexture:SetVertexColor(.3, .3, .3)
	...
	local unitPointBottom = ENEMYFRAMESPLAYERDATA['layout'] == 'hblock' and maxUnits - 4 or ENEMYFRAMESPLAYERDATA['layout'] == 'vblock' and (math.fmod(maxUnits,2) == 0 and maxUnits - 1 or math.fmod(maxUnits,2) ~= 0 and maxUnits) or maxUnits < ENEMYFRAMESPLAYERDATA['groupsize'] and maxUnits or ENEMYFRAMESPLAYERDATA['groupsize']
...
local getPrioBuff = function(buffList ,currentTime)
	if  not buffList then return end
	local prioIndex = 0
	local prioValue = 0
	
	for i, buff in ipairs(buffList) do
		if prioValue < buff.prio then
			if buff.timeEnd > currentTime then	-- checking for timeend without having to do maintenance
				prioIndex = i
				prioValue = buff.prio
			end
		end
...
local function updateUnits()
	...
	if ktEndtime < currentTime then
		enemyFrame.raidTargetFrame:Hide()
	end
	-- raidicons menu
	if rtMenuEndtime < currentTime then
		enemyFrame.raidTargetMenu:Hide()
	end
	...
	for k, v in pairs(playerList) do
		...
		local castInfo = SPELLCASTINGCOREgetCast(v['name'])
		if castInfo ~= nil and v['nearby'] then --and castInfo.timeEnd >= currentTime then
			units[i].castbar:SetMinMaxValues(0, castInfo.timeEnd - castInfo.timeStart)
			
			if castInfo.inverse then
				units[i].castbar:SetValue(mod((castInfo.timeEnd - currentTime), castInfo.timeEnd - castInfo.timeStart))
			else
				units[i].castbar:SetValue(mod((currentTime - castInfo.timeStart), castInfo.timeEnd - castInfo.timeStart))					
			end
...
function ENEMYFRAMESInitialize(maxUnits, isBG)
	...
	if maxUnits then
		...
		if insideBG and GetInstanceInfo() == 'Warsong Gulch' then
