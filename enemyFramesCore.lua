...
local nextPlayerCheck = 3--6	-- timer since last seen in seconds
...
local function addNearbyPlayers(players)
	...
	local nextCheck = currentTime + nextPlayerCheck
		...
		if insideBG then
			if playerList[v['name']] then
				--if not playerList[v['name']]['nearby'] or playerList[v['name']]['health'] 	~= v['health'] or playerList[v['name']]['mana'] ~= v['mana'] then
					...
					--if v['buffList'] then		playerList[v['name']]['buffList']	= v['buffList'] 		end
					--playerList[v['name']]['buffList']	= v['buffList']
					--if v['castData'] then		playerList[v['name']]['castData']	= v['castData']			end
					--playerList[v['name']]['castData']	= v['castData']
			...
			--if currentTime > enemyNearbyRefresh then
...
local function verifyUnitInfo(unit) --/run print(UnitIsPlayer('target') and 'true' or 'false')
	if UnitExists(unit) and UnitIsPlayer(unit) and UnitFactionGroup(unit) ~= playerFaction then --UnitIsEnemy(unit, 'player') then
		...
		local buffList = {}
		--local ct = 1
		local buff, debuffType, duration, expirationTime
		for i=1, 2 do
			for j=1, buffLimits[i] do
				if i == 1 then
					buff, _, icon, _, debuffType, duration, expirationTime = UnitBuff(unit, j)
				else
					buff, _, icon, _, debuffType, duration, expirationTime = UnitDebuff(unit, j)
				end
				...
				if buffData and buffData['prio'] and buffData['prio'] > 0 and duration > 0 then				
					...
					buffEntry.prio		= buffData['prio'] and buffData['prio'] or 0
					...
					buffList[buff] 		= buffEntry
					--ct = ct + 1
				end
			end
		end
		SPELLCASTINGCOREgetSetBuffs(u['name'], buffList)
		--u['buffList'] = buffList
		
		-- getcasting
		local channel, spell, icon, startTimeMS, endTimeMS, interruptable		
		for i = 1, 2 do
			if i == 1 then
				spell, _, _, icon, startTimeMS, endTimeMS, _, interruptable  = UnitChannelInfo(unit)
				interruptable = false and interruptable or true	-- i dunno lol
				channel = spell
			else
				spell, _, _, icon, startTimeMS, endTimeMS, _, interruptable  = UnitCastingInfo(unit)
			end
				
			if spell then
				local spellTarget = UnitExists(unit .. 'target') and UnitName(unit .. 'target') or nil
				SPELLCASTINGCOREupdateCast(u['name'], endTimeMS / 1000, interruptable, spellTarget)
				if not interruptable then print (spell .. ' is not interruptable') end
			end	
		end
		if not channel and not spell then SPELLCASTINGCOREremoveCast(u['name']) end
...
local function updatePlayerListInfo()
	...
	for k, v in pairs(playerList) do
		...
		local bList = {}
		if v['buffList'] then
			for i, buff in pairs(v['buffList']) do
				if buff.timeEnd > currentTime then	
					bList[i] = buff
				end
			end
		end
		...
		--cast validation
		if v['castData'] and v['castData'].timeEnd < currentTime then
			v['castData'] = nil
		end
		...
		if v['castData'] or tlength(v['buffList']) > 0 then	
		...
		-- outdoors
		if not insideBG then
			if not v['nearby'] and v['lastSeen'] and currentTime > v['lastSeen'] then
				playerList[v['name']] 	= nil
				refreshUnits 			= true				
			end
		end		
...
local function globalNearbyMaintenance()
	local currentTime = GetTime()
	local nextSeen	= currentTime + playerOutdoorLastseen
	for k, v in pairs(playerList) do
		-- remove inactive player
		if v['nextCheck'] and v['nearby'] then
			if currentTime > v['nextCheck'] then	
				refreshUnits 	= true 	
				v['nearby'] 	= false
...
local function initializeValues()
	...
	local maxUnits = bgs[GetInstanceInfo()] and bgs[GetInstanceInfo()] or ENEMYFRAMESPLAYERDATA['enableOutdoors'] and maxUnitsDisplayed or nil
	if maxUnits then
		--
		insideBG = bgs[GetInstanceInfo()] and true or false
